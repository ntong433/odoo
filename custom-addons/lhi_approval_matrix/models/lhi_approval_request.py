...
class LhiApprovalRequest(models.Model):
    ...
    name = fields.Char(string='Request ID', required=True, copy=False, default=lambda self: _('New'))
    ...
    matrix_id = fields.Many2one('lhi.approval.matrix', string='Approval Matrix', readonly=True)
    ...
    creator_id = fields.Many2one('res.users', string='Creator/Initiator', required=True, default=lambda self: self.env.user)
    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda self: self.env.company)

    line_ids = fields.One2many('lhi.approval.request.line', 'request_id', string='Approval Steps', copy=False)
    current_line_id = fields.Many2one('lhi.approval.request.line', string='Current Step', compute='_compute_current_line', store=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('lhi.approval.request') or _('New')
        return super(LhiApprovalRequest, self).create(vals_list)

    @api.depends('line_ids', 'line_ids.state')
    def _compute_current_line(self):
        for record in self:
            current = record.line_ids.filtered(lambda l: l.state == 'pending')
    ...
    def action_prepare(self):
        ...
        for line in matrix.line_ids:
            ...
            if not approver_users:
                raise UserError(
                    _("Approval stage '%s' has no eligible active approver.") % line.name
                )
    ...
    def action_approve(self, notes=None):
        ...
        if stage_complete:
            ...
            # Notify chatter
            self.message_post(body=_("Stage '%s' approved by %s.") % (current_line.name, user.name))
            
            # Find next stage
            next_line = self.line_ids.filtered(lambda l: l.state == 'pending')
            if not next_line:
    ...
    def action_reject(self, notes=None):
        ...
        self.message_post(body=_("Request rejected at stage '%s' by %s. Notes: %s") % (current_line.name, user.name, notes or ''))
    ...
    def action_return_for_correction(self, notes=None):
        ...
        self.message_post(body=_("Request returned for correction by %s. Notes: %s") % (user.name, notes or ''))
    ...
    def check_and_escalate_timeouts(self):
        ...
        for req in under_review_reqs:
            ...
            if line and line.timeout_days > 0:
                ...
                histories = line.history_ids.sorted(key=lambda h: h.action_date, reverse=True)
                if histories:
                ...
                if now > limit_date:
                    if line.escalation_user_id:
                        ...
                        line.write({
                            'approver_ids': [(4, line.escalation_user_id.id)]
                        })
                        req.message_post(body=_("Approval step '%s' timed out. Escalated to %s.") % (line.name, line.escalation_user_id.name))
                        # Log history
                        self.env['lhi.approval.history'].create({
                        ...
                        req.message_post(body=_("Approval step '%s' timed out. Request expired.") % line.name)
    ...
    def _update_source_document(self, target_state):
        ...
        try:
            ...
            if source_rec.exists():
                message = _("Approval Request %s state changed to %s.") % (self.name, target_state)
...
class LhiApprovalRequestLine(models.Model):
    ...
    request_id = fields.Many2one('lhi.approval.request', string='Approval Request', ondelete='cascade', required=True)
    matrix_line_id = fields.Many2one('lhi.approval.matrix.line', string='Configured Line', ondelete='set null')
    ...
    approver_ids = fields.Many2many('res.users', 'lhi_approval_req_line_approvers_rel', 'line_id', 'user_id', string='Eligible Approvers')
    approved_user_ids = fields.Many2many('res.users', 'lhi_approval_req_line_approved_rel', 'line_id', 'user_id', string='Approved By')
    ...
    history_ids = fields.One2many('lhi.approval.history', 'request_line_id', string='History')
...
class LhiApprovalHistory(models.Model):
    ...
    request_line_id = fields.Many2one('lhi.approval.request.line', string='Approval Step', ondelete='cascade', required=True)
