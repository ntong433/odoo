...
class LhiMemoIntegrationContracts(models.AbstractModel):
    ...
    def validate_all_contracts(self):
        ...
        if matrix_model is None or not hasattr(
            matrix_model, "_lhi_get_memo_approval_route"
        ):
            errors.append(
                _("lhi_approval_matrix does not expose _lhi_get_memo_approval_route.")
            )
        else:
        ...
        if item_model is None or not hasattr(
            item_model, "_lhi_prepare_and_confirm_memo_document"
        ):
            errors.append(
                _(
                    "lhi_sharepoint_storage does not expose "
                    "_lhi_prepare_and_confirm_memo_document."
                )
            )
        else:
        ...
        if request_model is None or not hasattr(
            request_model, "_lhi_create_memo_signature_draft"
        ):
            errors.append(
                _(
                    "lhi_signature_bridge does not expose "
        ...
        if user_model is None or not hasattr(
            user_model, "_lhi_get_memo_identity_contract"
        ):
            errors.append(
                _(
                    "lhi_entra_identity_sync does not expose "
                    "_lhi_get_memo_identity_contract."
                )
            )
        else:
